...
def gini_popadj(vector):
    """Added by MT to implement https://doi.org/10.1016/j.econlet.2019.108789
    Equivalent to Gini with population-size adjustement = G*n/(n-1)
    """
    n = vector.size
    return gini(vector) * n / (n - 1)
...
def mad(vector):
    mean = np.mean(vector)
    return np.mean(np.abs(vector - mean))